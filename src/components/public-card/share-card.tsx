...
export default function ShareCard({ card, url, children }: ShareCardProps) {
  ...
  const handleNativeShare = async () => {
    if (navigator.share) {
      try {
        await navigator.share({ title: `${card.name} | Smart Visiting Card`, text, url });
        return;
      } catch {
        // user cancelled
      }
      ...
      {open && (
        <div className="mt-3 p-5 rounded-2xl border bg-card/95 backdrop-blur shadow-xl animate-scale-in">
          ...
          <div className="flex flex-wrap gap-4 justify-center">
            ...
            <a
              href={`mailto:?subject=${encodeURIComponent(`${card.name} | Smart Visiting Card`)}&body=${encodeURIComponent(
                text + "\n\n" + url
              )}`}
              className="flex flex-col items-center gap-1.5 group"
              onClick={() => setOpen(false)}
